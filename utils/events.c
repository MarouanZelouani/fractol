...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/12 03:14:06 by mzelouan          #+#    #+#             */
/*   Updated: 2024/05/12 04:26:52 by mzelouan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	events_handler(int keycode, t_param *param)
{
	...
	if (!ft_strncmp("julia", param->fractal.name, 5))
		draw_julia(&param->img, &param->plan, param->move, *param);
...
static void	draw(t_param *param)
{
	if (!ft_strncmp("julia", param->fractal.name, 5))
		draw_julia(&param->img, &param->plan, param->move, *param);
	else if (!ft_strncmp("mandelbrot", param->fractal.name, 10))
